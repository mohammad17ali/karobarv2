...
import '../services/update_orders.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  List<Map<String, dynamic>> get _activeOrders {
    final active = _orders
      ...
      'Status': order['Status'],
      'AirtableRecordIds': order['AirtableRecordIds'],
    }).toList();

    active.sort((a, b) => b['OrderNum'].compareTo(a['OrderNum']));
    return active;
  }

  void _handlePayOrder(Map<String, dynamic> order) async {
    final List<String>? airtableRecordIds = (order['AirtableRecordIds'] as List?)?.map((e) => e.toString()).toList();

    if (airtableRecordIds == null || airtableRecordIds.isEmpty) {
      print('Error: AirtableRecordIds not found or empty for order ${order['OrderNum']}');
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Cannot process payment: Order IDs missing.'),
          backgroundColor: Colors.red,
        ),
      );
      _hideOrderDetails();
    ...
    final bool success = await UpdateOrders.completeOrder(airtableRecordIds); // Use completeOrder for payment

    if (success) {
      setState(() {
        final index = _orders.indexWhere((o) => o['OrderNum'] == order['OrderNum']);
        if (index != -1) {
          _orders[index]['Status'] = 'Completed'; // Update local state
        }
      });
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Payment successful! Order marked as Completed.'),
          backgroundColor: Colors.green,
        ),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Failed to update order status after payment.'),
          backgroundColor: Colors.red,
        ),
      );
    }
  ...
  void _handleCancelOrder(Map<String, dynamic> order) async {
    ...
    if (airtableRecordIds == null || airtableRecordIds.isEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Cannot cancel order: Order IDs missing.'),
          backgroundColor: Colors.red,
        ),
      );
      _hideOrderDetails();
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Cancelling order: ${order['OrderNum']}...'),
        backgroundColor: Colors.orange,
      ),
    );

    final bool success = await UpdateOrders.cancelOrder(airtableRecordIds);
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Order ${order['OrderNum']} cancelled successfully!'),
          backgroundColor: Colors.red,
        ),
      );
    } else {
  ...
  Widget _buildPortraitSearchBar() {
    return Container(
      color: AppColors.primary,
      padding: EdgeInsets.fromLTRB(16.w, 0, 0, 12.h), // Remove right padding
      child: Row(
        children: [
          Expanded(
            child: Container(
          ...
          GestureDetector(
            onTap: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => const DashboardPage(), // Make sure to import dashboard.dart
                ),
              );
            },
            child: Container(
      ...
      child: ElevatedButton(
        ...
        style: ElevatedButton.styleFrom(
          ...
          padding: EdgeInsets.symmetric(
            ...
            vertical: _useLandscapeLayout ? 8.h : 5,
          ),
        ),
        child: Text(
          title,
  ...
  Widget _buildCategoryBar() {
    return Container(
      height: _useLandscapeLayout ? 55.h : 45.h,
      color: AppColors.primary,
      child: ListView(
        scrollDirection: Axis.horizontal,
        padding: EdgeInsets.symmetric(horizontal: _useLandscapeLayout ? 8.w : 3.w),
      ...
      children: [
                ...
                child: Container(
                  decoration: BoxDecoration(
                    borderRadius: BorderRadius.vertical(
                      top: Radius.circular(_useLandscapeLayout ? 16 : 14),
                    ),
        ...
        if (isAdded) ...[
          Positioned(
            ...
            top: 2,
            child: CircleAvatar(
              radius: _useLandscapeLayout ? 20 : 15,
          ...
          Positioned(
            ...
            child: GestureDetector(
              ...
              child: CircleAvatar(
                radius: _useLandscapeLayout ? 24 : 18,
                        ...
                        Column(
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              child: _selectedOrder!['ItemNames'] != null &&
                                  (_selectedOrder!['ItemNames'] as List).isNotEmpty
                                  ? ListView.separated(
                                shrinkWrap: true,
                                itemCount: (_selectedOrder!['ItemNames'] as List).length,
                                separatorBuilder: (context, index) => SizedBox(height: 4.h),
                                itemBuilder: (context, index) {
                                  final itemName = (_selectedOrder!['ItemNames'] as List)[index];
                                  return Row(
                                    children: [
                                      Container(
                                        width: 6.w,
                              ...
                              final result = await Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) => PaymentsPage(
                                    totalAmount: (_selectedOrder!['Amount'] as num).toDouble(),
                                    orderNumber: _selectedOrder!['OrderNum'] as int,
                                  ),
                                ),
                              );

                              if (result == true) {
                                _handlePayOrder(_selectedOrder!);
                                await _loadOrders();
                              }
                            } catch (e) {
                              ScaffoldMessenger.of(context).showSnackBar(
